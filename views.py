...
@app.route("/unit_api/plugins/installed/<filename>", methods=["GET"])
...
@app.route("/unit_api/versions/app", methods=["GET"])
...
if am_I_leader():
    ...
    def get_logs_for_unit_and_experiment(
        experiment: str, pioreactor_unit: str
    ) -> ResponseReturnValue:
    ...
    def get_fallback_time_series(
        data_source: str, experiment: str, column: str
    ) -> ResponseReturnValue:
        args = request.args
        try:
            lookback = float(args.get("lookback", 4.0))
    ...
    def get_job_contrib() -> ResponseReturnValue:
        try:
            ...
            files = sorted(job_path_default.glob("*.y*ml")) + sorted(
                job_path_plugins.glob("*.y*ml")
            )
            ...
            for file in files:
                try:
                    decoded_yaml = yaml_decode(
                        file.read_bytes(), type=structs.BackgroundJobDescriptor
                    )
    ...
    def get_charts_contrib() -> ResponseReturnValue:
        try:
            ...
            chart_path_plugins = (
                Path(env["DOT_PIOREACTOR"]) / "plugins" / "ui" / "contrib" / "charts"
            )
            ...
            for file in files:
                try:
                    ...
                    publish_to_error_log(
                        f"Yaml error in {Path(file).name}: {e}", "get_charts_contrib"
                    )
